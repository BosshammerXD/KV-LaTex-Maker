...
import IterTools
from .KVInputHandler import KVInputHandler
from . import KVUtils
...
class KVManager:
    ...
    def __init__(self, canvas: Canvas) -> None:
        ...
        self.__kvdata = KVData(self.__kv_markings, VALUES, VARS.split(","))
        self.__kvdata.add_marking(self.current_col.get(), self.__marking_id_generator.generate_id())
        self.__kvdata.selected = 0
        self.input_handler: KVInputHandler = KVInputHandler(canvas, self, self.__kvdata.get_selected_marking)

    def get_kv_string(self) -> str:
        return get_kv_string(self.__kvdata, self.title.get())

    def color_changed(self, new_color: str):
        marking = self.__kvdata.get_selected_marking()
        marking.latex_color = new_color
        marking.tkinter_color = DYNAMIC.Colors[new_color]
        self.__kv_markings.set_color(marking.TAG, marking.tkinter_color)

    def new_marking(self) -> None:
        current_indices = self.__kvdata.get_selected_marking().indices
        if len(current_indices) == 0:
            return

        self.current_col.set(next(self.__colors))
        
        self.__kvdata.add_marking(self.current_col.get(), self.__marking_id_generator.generate_id(), self.__kvdata.selected + 1)
        self.__kvdata.selected += 1
        self.__kv_markings.update_selected(self.__kvdata.selected)
        

    def different_marking(self, offset: int) -> None:
        current_indices = self.__kvdata.get_selected_marking().indices
        if len(current_indices) == 0 and self.__kvdata.len_markings > 1:
            self.remove_marking(
                self.__kvdata.selected,
                self.__kvdata.get_selected_marking()
            )
            self.__kvdata.remove_marking(self.__kvdata.selected)
        else:
            self.__kvdata.selected += offset

        self.current_col.set(self.__kvdata.get_selected_marking().latex_color)
        self.__kv_markings.update_selected(self.__kvdata.selected)
    ...
    def remove_selected_marking(self):
        self.remove_marking(self.__kvdata.selected, self.__kvdata.get_selected_marking())

    def remove_marking(self, index: int, marking: Marking):
        marking.indices.clear()
        self.__kv_markings.update_marking(index, marking.drawables, marking.tkinter_color)
    ...
    def update_sizes(self, width: int, height: int) -> None:
        ...
        self.__kv_vars.update(self.__kvdata.vars)
        self.__kv_grid.update(self.__kvdata.width, self.__kvdata.height)
        self.__kv_values.update(self.__kvdata.vals)
        self.__kv_indices.update(2**self.__kvdata.get_num_vars())
        self.draw_all()
    ...
    def draw_all(self) -> None:
        ...
        self.__kv_markings.draw(self.__kv_grid, self.__kvdata.markings)
    ...
    def update_selected_marking(self):
        ...
        self.__kv_markings.update_marking(self.__kvdata.selected, marking.drawables, marking.tkinter_color)
        self.__kv_markings.draw_marking(self.__kv_grid, self.__kvdata.selected, marking.drawables)
    
    def update_vals(self, new_vals: str) -> None:
        self.__kvdata.vals = new_vals
        self.__kv_values.update(new_vals)
        self.__kv_values.draw(self.__kv_grid)
    
    def update_vars(self, new_vars: list[str]) -> None:
        if new_vars != self.__kvdata.vars:
            self.__kvdata.vars = new_vars
            num_left_vars = self.__kvdata.get_num_vars() // 2
            num_top_vars = self.__kvdata.get_num_vars() - num_left_vars
